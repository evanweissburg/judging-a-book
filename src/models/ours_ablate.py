...
class OursAblateModelConfig(models.ModelConfig):
    model_type: models.ModelType = models.ModelType.OursAblate
    lr: float = 1e-5
    ...
    def build(self, device: str) -> List[Tuple[models.Model, float]]:
        ...
        row_tokens, max_sentence_len = _tokenize(rows, name)
        row_pairs = _pair_rows(rows, row_tokens, pairs, name)
        dataset = Dataset(row_pairs)
        build_model = lambda: OursAblateModel(self, max_sentence_len).to(device)
        return self.training_scheme(torch_device, self, build_model, dataset, name)


def _tokenize(rows: pd.DataFrame, name: str) -> Tuple[RowTokens, int]:
    ...
    return row_tokens, max_sentence_len
...
class OursAblateModel(torch.nn.Module):
    def __init__(self, config: OursAblateModelConfig, max_sentence_len: int):
        super().__init__()
        self._max_sentence_len = max_sentence_len

        self.attn = torch.nn.MultiheadAttention(300, config.n_heads, config.dropout)
        self.linear = torch.nn.Sequential(
            torch.nn.Linear(max_sentence_len * 300, 2048),
            torch.nn.ReLU(),
